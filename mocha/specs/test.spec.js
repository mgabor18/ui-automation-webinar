...
const DataProvider = require("../data/app");
const dataProvider = new DataProvider();
const careerPage = new CareerPage();
const searchResultPage = new SearchResultPage();
const jobDescriptionPage = new JobDescriptionPage();

//példányosítás után ráhívom a metódusait
dataProvider
  .putData(data)
  .then(async () => {
    //kiolvasom az adatokat
    dataProvider.fData = await dataProvider.getData();
  })
  .then(async () => {
    //törlés
    await dataProvider.deleteData(data);
  })
  .then(async () => {
    //itt ki is írja egyébként az adatokat, tehát azok megvannak
    console.log(dataProvider.fData);
    //majd ezután ez nem fut le, pontosabban hamarabb mint a console log és simán visszajön 0 testtel?!
    //gondolom ehhez kéne a run() esetleg, de nemtudom hová tegyem mert sehol nem jó
    dataProvider.fData.forEach(async testData => {
        ...
        describe("Search form", () => {
          ...
          describe("Searching", () => {
            ...
            it(`should have ${testData.positionName} job with ${testData.country} location`, async () => {
              expect(
                await searchResultPage.getJobElementText(searchResultPage.jobLocation(testData.nthJob))
              ).to.include(testData.country.toUpperCase());
            });

            it(`should have ${testData.positionName} job with description`, async () => {
              expect(searchResultPage.isElementVisible(searchResultPage.jobDescription(testData.nthJob))).to.eventually
                .be.true;
            });

            it(`should have apply button for ${testData.positionName} job`, async () => {
              expect(searchResultPage.isElementVisible(searchResultPage.jobApplyButton(testData.nthJob))).to.eventually
                .be.true;
            });
          });
